...
function AccordionSection({ title, children, defaultOpen = true }) {
  ...
  return (
    <div className="border-b border-surface-5 last:border-b-0">
      <button
        type="button"
...
function TypographyControls({ block, update, columns, savedColors, addSavedColor, customFonts, addCustomFont, removeCustomFont, fontInputRef }) {
  const allFonts = [
  ...
  const handleFontUpload = (e) => {
    ...
    reader.onload = (ev) => { addCustomFont({ name, src: ev.target.result, format }); update({ fontFamily: name }) }
      ...
      <Field label="Police">
        <div className="flex gap-1.5">
          <Select
            ...
            className="flex-1"
          />
          <button className="btn-icon bg-surface-3 border border-surface-5 rounded-lg px-2"
            title="Charger une police (.ttf, .woff, .woff2)"
            onClick={() => fontInputRef.current?.click()}>
            <Upload size={12} />
          </button>
          <input ref={fontInputRef} type="file" className="hidden"
            accept=".ttf,.woff,.woff2,.otf" onChange={handleFontUpload} />
        </div>
        ...
        <div className="flex gap-1">
          {[['left','G'], ['center','C'], ['right','D'], ['justify','J']].map(([a, icon]) => (
            <button key={a}
              ...
              title={a} onClick={() => update({ align: a })}>{icon}</button>
          ))}
        </div>
      </Field>
        ...
        <div className="flex gap-1">
          {[['top','Haut'], ['center','Centre'], ['bottom','Bas']].map(([v, lbl]) => (
            <button key={v}
              ...
              onClick={() => update({ vAlign: v })}>{lbl}</button>
          ))}
        </div>
      </Field>
...
function BackgroundControls({ block, update, savedColors, addSavedColor }) {
  return (
    <div className="flex flex-col gap-3">
      <Field label="Couleur de fond">
        <div className="flex items-center gap-2">
          <ColorPicker color={block.bgColor ?? 'transparent'} onChange={c => update({ bgColor: c })} savedColors={savedColors} onSaveColor={addSavedColor} />
              ...
              {[['full','Vignette'], ['fit','Texte']].map(([mode, lbl]) => (
                <button key={mode}
                  className={`flex-1 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                    (block.widthMode ?? 'full') === mode ? 'border-accent bg-accent/15 text-accent' : 'border-surface-5 bg-surface-3 text-gray-500 hover:border-surface-6'
                  }`}
                  onClick={() => update({ widthMode: mode })}>{lbl}</button>
              ))}
            </div>
          </Field>
...
function SpacingControls({ block, update }) {
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-2 gap-2">
      ...
      <Field label="Lignes max">
        <div className="flex items-center gap-2">
          <NumberInput value={block.maxLines ?? 1} onChange={v => update({ maxLines: Math.max(1, Math.round(v)) })} min={1} max={8} step={1} />
...
function TextProps({ block, update, columns, savedColors, addSavedColor, customFonts, addCustomFont, removeCustomFont, fontInputRef }) {
  ...
  const addColumn    = (col) => { if (!col) return; update({ columns: [...mappedCols, col] }); setNewCol(''); setShowAdd(false) }
  ...
  return (
    <AccordionSection title="Colonnes liées" defaultOpen={true}>
      {mappedCols.length === 0 && (
      ...
      {available.length > 0 && showAdd ? (
        <div className="flex gap-1.5">
          <Select value={newCol} onChange={v => addColumn(v)}
            options={available.map(c => ({ value: c, label: c }))}
            placeholder="Choisir une colonne..." className="flex-1 text-xs" />
          <button className="btn-icon bg-surface-3 border border-surface-5 rounded-lg px-2" onClick={() => setShowAdd(false)}><X size={12} /></button>
        </div>
      ) : available.length > 0 ? (
        <button className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-surface-6 text-xs text-gray-500 hover:text-gray-300 hover:border-accent/50 transition-colors"
      ...
      {mappedCols.length > 1 && (
        <div className="mt-3">
          <Field label="Separateur entre colonnes">
            <input className="input text-xs" value={block.separator ?? ' '}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <Field label="Prefixe">
          <input className="input text-xs" value={block.prefix ?? ''}
            onChange={e => update({ prefix: e.target.value })}
            placeholder='ex: Prix: ' />
        </Field>
        <Field label="Suffixe">
          <input className="input text-xs" value={block.suffix ?? ''}
            onChange={e => update({ suffix: e.target.value })}
            placeholder='ex: EUR HTVA' />
        </Field>
      </div>
    </AccordionSection>
  )
...
function ImageProps({ block, update }) {
  return (
    <AccordionSection title="Image produit" defaultOpen={true}>
      <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-3">
        ...
        {block.conditionOperator !== 'notempty' && (
          <Field label="Valeur attendue">
            <input className="input text-xs" value={block.conditionValue ?? ''}
              onChange={e => update({ conditionValue: e.target.value })}
              placeholder="ex: oui, bio, *…" />
          </Field>
        )}
...
function SeparatorProps({ block, update, savedColors, addSavedColor }) {
  return (
    <AccordionSection title="Séparateur" defaultOpen={true}>
      <div className="flex flex-col gap-3">
        ...
        <Field label="Largeur">
          <input className="input text-xs" value={block.separatorWidth ?? '100%'}
            onChange={e => update({ separatorWidth: e.target.value })}
            placeholder="100% ou 80mm…" />
        </Field>
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex flex-col px-4 pt-2">
        ...
        {/* ── Position ───────────────────────────────── */}
        <AccordionSection title="Position" defaultOpen={true}>
        ...
        {block.type !== 'image' && (
          <TextProps block={block} update={update} columns={columns}
            savedColors={savedColors} addSavedColor={addSavedColor}
        ...
        {/* ── Typography (text/static only) ──────────── */}
        {(block.type === 'text' || block.type === 'static') && (
          <AccordionSection title="Typographie" defaultOpen={true}>
            <TypographyControls block={block} update={update} columns={columns}
              savedColors={savedColors} addSavedColor={addSavedColor}
              customFonts={customFonts} addCustomFont={addCustomFont} removeCustomFont={removeCustomFont}
              fontInputRef={fontInputRef} />
          </AccordionSection>
        ...
        {/* ── Background (text/static/separator) ─────── */}
        {(block.type === 'text' || block.type === 'static' || block.type === 'separator') && (
          <AccordionSection title="Fond et bordures" defaultOpen={false}>
            <BackgroundControls block={block} update={update} savedColors={savedColors} addSavedColor={addSavedColor} />
          </AccordionSection>
        )}

        {/* ── Spacing (text/static only) ─────────────── */}
        {(block.type === 'text' || block.type === 'static') && (
          <AccordionSection title="Espacement interne" defaultOpen={false}>
            <SpacingControls block={block} update={update} />
          </AccordionSection>
        )}
      </div>
    </div>
